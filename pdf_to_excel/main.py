    ...
    for root, dirs, files in os.walk(source_dir):
        for file in files:
            if file.endswith('.pdf'):
                ...
                dst_file = os.path.join('data1', file)
...
def save_excel(data: list) -> None:
    ...
    if not os.path.exists('results/result_data1.xlsx'):
        # Если файл не существует, создаем его с пустым DataFrame
        with ExcelWriter('results/result_data1.xlsx', mode='w') as writer:
            DataFrame().to_excel(writer, sheet_name='data', index=False)

    # Загружаем данные из файла
    df = read_excel('results/result_data1.xlsx', sheet_name='data')
    ...
    with ExcelWriter('results/result_data1.xlsx', mode='a', if_sheet_exists='overlay') as writer:
