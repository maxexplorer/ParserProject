...
category_urls_list = [
    "https://www.msn.kg/ru/news/rubrics/?rubric=56",
    "https://www.msn.kg/ru/news/rubrics/?rubric=1-",
    "https://www.msn.kg/ru/news/rubrics/?rubric=2",
    "https://www.msn.kg/ru/news/rubrics/?rubric=6"
]
    ...
    with requests.Session() as session:
        for i, category_url in enumerate(category_urls_list, 1):
            ...
            for page in range(1, pages + 1):
                ...
                try:
                    data = soup.find('div', class_='contentbox').find_all('h3')
...
def get_data(file_path: str, headers: dict) -> list:
    ...
    with requests.Session() as session:
        for i, article_url in enumerate(article_urls_list, 1):
            ...
            try:
                data = soup.find('div', id='printdiv')
            except Exception as ex:
                print(f'data: {ex}')
                continue
            try:
                article_title = data.find('h2').text.strip()
            except Exception:
                article_title = ''
...
def save_excel(data: list, category_title: str) -> None:
    ...
    if not os.path.exists('results/MCH.xlsx'):
        # Если файл не существует, создаем его с пустым DataFrame
        with ExcelWriter('results/MCH.xlsx', mode='w') as writer:
            DataFrame().to_excel(writer, sheet_name=category_title, index=False)

    dataframe = DataFrame(data)

    with ExcelWriter('results/MCH.xlsx', if_sheet_exists='replace', mode='a') as writer:
        dataframe.to_excel(writer, sheet_name=category_title, index=False)
