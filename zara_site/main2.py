...
from data.data import id_category_list_home
...
def get_id_categories(headers: dict, params: dict) -> None:
    ...
    with Session() as session:
        try:
            response = session.get(
                'https://www.zara.com/tr/en/categories',
                headers=headers,
                params=params,
                timeout=60
            )
    ...
    for main_category_item in main_category_items[3:4]:
...
def get_id_products(id_categories_list: list, headers: dict, params: dict, species: str, brand: str, region: str,
    ...
    directory = 'data'
    file_path = f'{directory}/id_products_list_{species}_{brand}_{region}.txt'
...
def main():
    ...
    products_data_list = get_id_products(id_categories_list=id_category_list_home, headers=headers, params=params,
                                         species='home', brand=brand, region=region, id_region=id_region)
    get_products_array(products_data_list=products_data_list, headers=headers, species='home', brand=brand,
