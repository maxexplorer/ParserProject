...
def extract_value(soup: BeautifulSoup, label: str) -> float | None:
    try:
        value_text = soup.find('span', string=re.compile(label)).find_next().get_text(strip=True)
        return float(value_text.replace(',', '.')) / 1000  # на случай запятой
    except Exception:
        return None
...
def get_product_data(article: str, headers: dict, session: Session, ):
    product_url = f"https://elbrus-b2b.ru/search?pcode={article}"
    ...
    try:
        short_url = soup.find('div', class_='searchTopLinks').find_next_sibling('a').get('href')
        review_url = f'https://elbrus-b2b.ru{short_url}'
    except Exception as ex:
        review_url = None
        print(f'not review_url: {product_url} - {ex}')
    ...
    height_m = extract_value(soup, 'Высота упаковки, мм:')

    length_m = extract_value(soup, 'Длина упаковки, мм:')

    width_m = extract_value(soup, 'Ширина упаковки, мм:')

    if None in (height_m, length_m, width_m):
        return None

    volume_m3 = height_m * length_m * width_m
    ...
    with Session() as session:
        for data_file_path in excel_files:
            ...
            for sheet_name in wb.sheetnames:
                ws = wb[sheet_name]
                headers_excel = [cell.value for cell in ws[4]]

                volume_column_index = headers_excel.index('Volume')  # Ищем столбец 'Volume'
                ...
                for row in ws.iter_rows(min_row=5, max_col=len(headers)):
                    ...
                    volume_m3 = get_product_data(article=article, headers=headers, session=session)
                ...
                wb.save(data_file_path)  # сохраняем после обработки текущего листа
