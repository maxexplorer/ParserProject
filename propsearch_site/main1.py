...
def get_data(session, data_list):
    result_list = []
...
def save_csv(data):
    ...
    with open(f'data/data_exceptions_list.csv', 'w', encoding='utf-8', newline='') as file:
    ...
    with open('data/data_exceptions_list.csv', 'a', encoding='utf-8', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(
            data
        )
...
def main():
    # with open('data/exceptions_list.csv', 'r', encoding='utf-8') as file:
    #     reader = csv.reader(file)
    #     data_list = list(reader)
    with requests.Session() as session:
        data = get_data(session=session, data_list=["https://propsearch.ae/dubai/bay-square-building-2"])
        save_csv(data)
