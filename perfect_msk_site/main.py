    ...
    with requests.Session() as session:
        for id, title, url in data_list:
            try:
                ...
                continue
            try:
                title_site = soup.find(class_='product-header__title').text.strip()
            except Exception:
                title_site = None
            try:
                price = soup.find(class_='prod_price prod_big_price').find_next().text.strip()
            except Exception:
                price = None
...
def main():
    with open('data/perfect_msk.csv', 'r', encoding='cp1251') as file:
